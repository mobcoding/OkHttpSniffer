...
Draft for maintainer review. Based on the source at commit `424e8bc`, reviewed on 2026-09-23. Confirm this statement against the final release before publishing it as a policy.
...
The plugin can open websites or request URLs when the user selects an external-link action. Those destinations apply their own data policies. In the reviewed version, the localization and support buttons open LocaleBro and the upstream author's donation page respectively. Update this paragraph if those actions change before release.
